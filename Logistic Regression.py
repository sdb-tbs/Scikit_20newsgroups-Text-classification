...
""" Logistic Regression
""" 
text_clf = Pipeline([('tfidf', TfidfVectorizer(ngram_range=(1,2))),('clf',  LogisticRegression(random_state=42))])
...
param_grid = {'tfidf__ngram_range': [(1, 1), (1, 2)],'clf__C': np.logspace(-3,3,7),
              'clf__penalty': ['l1', 'l2']}
...
"""
start = time.time()
text_clf = Pipeline([('tfidf', TfidfVectorizer(ngram_range=(1,2))),('clf',  LogisticRegression(random_state=42, C =1000, penalty = 'l2' ) )])
text_clf.fit(twenty_train.data, twenty_train.target)
#Pipeline(...)
twenty_test = fetch_20newsgroups(subset = 'test', categories = categories, shuffle=True, random_state=42)
docs_test= twenty_test.data
